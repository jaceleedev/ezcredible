...
export default function CertificationPage() {
  return (
    <>
      ...
      {/* 02 — 대표 지시(2026-08-26): ISMRI를 전면 배치하고, 카드 전체를 ismri.org로 가는 링크로 */}
      <NumberedSection
        index={2}
        title="인증 파트너 ISMRI"
        aside={
          <a
            href={certificationPartner.url}
            target="_blank"
            rel="noreferrer"
            className="stage-cobalt group relative flex w-full flex-col justify-between gap-7 overflow-hidden rounded-[28px] p-8 text-white shadow-float transition-transform duration-300 ease-(--ease-out-quart) hover:-translate-y-1 lg:p-10"
          >
            ...
            <span className="relative inline-flex h-14 shrink-0 items-center justify-between gap-3 rounded-full bg-white px-7 text-base font-bold text-cobalt-700 shadow-button transition-colors duration-300 group-hover:bg-cobalt-50">
              <span>ismri.org 바로가기</span>
      ...
      <NumberedSection index={4} title="인증의 효과 · 솔루션">
