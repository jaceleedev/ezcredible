...
**남은 일: factoring.ezcredible.com을 `/liquidity-funds/receivables-factoring`으로 301/308 리다이렉트.**
지금은 같은 상품을 설명하는 페이지가 둘이라 검색·신뢰 양쪽에 손해다. 서브도메인이라 `next.config.ts`가 아니라
DNS/호스팅 쪽에서 처리해야 한다(카페24 접근 권한 필요).
...
- factoring.ezcredible.com(별도 골드 톤 랜딩) 유지/흡수 — **옛 랜딩이 더존 상품, 새 페이지가 기보 제도라 서로 다른 상품을 설명 중이다**(위 대조 절). 주력 상품을 먼저 정할 것
