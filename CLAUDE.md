...
완료: 디자인 시스템(`/design-system` 쇼케이스 라우트, noindex) · 홈(`/`) · **정책자금 4** · **유동성자금 3** · **성장 4** · **회사소개 4** = 서브페이지 15/17 — 2026 수치 반영, 4폭 검증, 빌드 통과
다음 순서:
1. 고객지원 2페이지 — 개인정보처리방침(기존 본문 그대로 이식) · 상담신청(폼 UI 먼저, 전송 API는 맨 마지막)
2. 404 페이지, `sitemap.ts`/`robots.ts`
3. 코덱스 이미지 교체(아래 리스트), 배포
4. **맨 마지막**: 상담 신청 폼 전송 + `/api/consultations` (전송 방식은 그때 Jace에게 확인 — 사이트의 유일한 기능이라 뒤로 미뤘다)
