...
완료: 디자인 시스템(`/design-system` 쇼케이스 라우트, noindex) · 홈(`/`) · **정책자금 4** · **유동성자금 3** · **성장 4**(기업신용평가·PG·VAN·기업인증) = 서브페이지 11/17 — 2026 수치 반영, 4폭 검증, 빌드 통과
다음 순서:
1. 회사소개 4페이지(회사소개·연혁·업무절차·오시는길) — 자금 페이지와 달리 `FundIntro` 없이 페이지별 고유 구성(타임라인·6단계·지도 링크)
2. 개인정보처리방침 — 기존 사이트 `support/privacy-policy` 본문을 그대로 이식(13조, 약 8,500자, 2023-08-31 제정)
3. 404 페이지, `sitemap.ts`/`robots.ts`
4. 코덱스 이미지 교체(아래 리스트), 배포
5. **맨 마지막**: 상담 신청 폼 + `/api/consultations` (전송 방식은 그때 Jace에게 확인 — 사이트의 유일한 기능이라 뒤로 미뤘다)
