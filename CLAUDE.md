...
**따라서 새 팩토링 페이지를 더존 중심으로 바꾸기로 하면 옛 랜딩의 수치를 가져오면 된다**(Jace가 실제 자료로 만든 값).
흡수하는 경우 factoring.ezcredible.com은 `/liquidity-funds/receivables-factoring`으로 301/308 리다이렉트할 것 —
지금은 두 페이지가 같은 상품을 서로 다른 조건으로 설명하고 있어 검색·신뢰 양쪽에 손해다.
